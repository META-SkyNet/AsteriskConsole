...
namespace Asterisk.NET.Manager
{
	...
	/// </summary>
	public class ManagerConnection : Asterisk.NET.Manager.IManagerConnection
	{
		#region Variables
		...
		private void reconnect(bool init)
		{
			...
			if (init)
				reconnectCount = 0;
			else if (reconnectCount++ > Common.DEFAULT_RECONNECT_RETRY_MAX)
				reconnectEnable = false;
			...
			if (reconnectEnable)
			{
				...
				{
					if (retryCount >= Common.DEFAULT_RECONNECT_RETRY_MAX)
						reconnectEnable = false;
					else
					{
						try
						{
							if (retryCount < Common.DEFAULT_RECONNECT_RETRY_FAST)
							{
								// Try to reconnect quite fast for the firt times
								// this succeeds if the server has just been restarted
#if LOGGER
								logger.Info("Reconnect delay : {0}, retry : {1}", Common.DEFAULT_RECONNECT_INTERVAL_FAST, retryCount);
#endif
								Thread.Sleep(Common.DEFAULT_RECONNECT_INTERVAL_FAST);
							}
							else
							{
								// slow down after unsuccessful attempts asuming a shutdown of the server
#if LOGGER
								logger.Info("Reconnect delay : {0}, retry : {1}", Common.DEFAULT_RECONNECT_INTERVAL_SLOW, retryCount);
#endif
								Thread.Sleep(Common.DEFAULT_RECONNECT_INTERVAL_SLOW);
							}
						}
		...
		public Response.ManagerResponse SendAction(ManagerAction action, int timeout)
		{
			...
			bool result = autoEvent.WaitOne(timeout <= 0 ? -1 : timeout, false);
